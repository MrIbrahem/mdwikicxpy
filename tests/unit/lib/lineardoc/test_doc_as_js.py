...
class TestLinearDoc:
    ...
    def test_should_be_possible_to_linearise_all_kind_of_html_inputs(self):
        ...
        for i in range(1, num_tests + 1):
            ...
            deep_equal(
                normalize(parser.builder.doc.dump_xml()),
                normalize(expected_xml),
                'Linearised structure'
            )
            deep_equal(
                normalize(parser.builder.doc.get_html()),
                normalize(expected_xhtml),
                'Reconstructed XHTML'
            )
    ...
    def test_should_be_possible_to_reconstruct_the_html_from_lineardoc(self):
        for test in trans_tests:
            ...
            text_block2 = text_block1.translate_tags(
                test['targetText'],
                test['rangeMappings']
            )
    ...
    def test_should_not_ignore_a_section_that_begins_with_an_inline_template_but_has_translatable_prose(self):
        ...
        assert not parser.builder.doc.is_ignorable_block(), 'Section starting with an inline transclusion followed by prose is translatable'
    ...
    def test_html_compaction_roundtrip_with_inline_style_content(self):
        ...
        deep_equal(
            not not extracted_data['2'].get('content'),
            True,
            'Content extracted for style tag'
        )
        deep_equal(
            not not extracted_data['3'].get('content'),
            True,
            'Content extracted for script tag'
        )
        deep_equal(
            not not extracted_data['5'].get('content'),
            True,
    ...
    def test_get_root_item_for_ignoring_blockspaces(self):
        ...
        deep_equal(
            root_item["name"],
            'p',
            'get_root_item should ignore the blockspaces in the beginning of the document'
        )
    ...
    def test_get_root_item_for_not_ignoring_non_whitespace_content_in_textblock(self):
        ...
        textblock = next(item for item in parsed_doc.items if getattr(item, 'type', None) == 'textblock')
        ...
        deep_equal(
            root_item,
            None,
            'get_root_item of textblock should consider non-whitespace content inside its textchunks'
        )
