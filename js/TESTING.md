...
```bash
node --test tests/unit/lib/segmentation/CXSegmenter.test.js
```
...
-   `npm run unittest` discovers and runs every `*.test.js` under the current
    ...
    ```bash
    cd js && node --test ../tests/unit/lib/segmentation/CXSegmenter.test.js
    ```
...
4. Runs `node --test ../tests/unit/lib/segmentation/CXSegmenter.test.js`.

## Current status

The suite **passes**: all 40 cases in `tests/unit/lib/segmentation/CXSegmenter.test.js`
